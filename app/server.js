...
// Настраиваем CORS
app.use(cors({
  origin: ['http://localhost:5173', 'http://127.0.0.1:5173', 'http://localhost:3000'], // URL вашего фронтенда
...
export const connectToMongoDB = async () => {
  try {
    // Если уже происходит подключение, дожидаемся его завершения
    if (isConnecting) {
      console.log('Подключение уже в процессе, ожидаем...');
      // Ждем, пока текущее подключение завершится
      for (let i = 0; i < 50; i++) {
    ...
    // Проверяем, активно ли уже соединение
    ...
    useUnifiedTopology: true,
      ...
      socketTimeoutMS: 60000, // Увеличиваем таймаут до 60 секунд
      family: 4, // Используем IPv4 вместо IPv6
      autoIndex: true, // Создаем индексы автоматически
      connectTimeoutMS: 15000, // Таймаут подключения
...
app.get('/api/products/:id', async (req, res) => {
    try {
        ...
        // Пытаемся найти продукт по ID с проекцией
        let product;
        try {
            product = await Product.findById(id).select('_id name price images ratingAvg ratingCount flavor firm description');
        } catch (findError) {
            console.error('Ошибка при поиске продукта по ID:', findError);
            // Если ошибка связана с неверным форматом ID, пробуем найти по строковому ID
            product = await mongoose.connection.db.collection('products')
                .findOne(
                    { _id: id },
